...
class TestCvpClientCC(TestCvpClient):
    ...
    def __init__(self, *args, **kwargs):
        super(TestCvpClientCC, self).__init__(*args, **kwargs)
        self.cc_id = None
    ...
    def change_control_opearions(self, methodToRun, cc_name="", cc_notes=""):
        if self.clnt.apiversion is None:
            self.api.get_cvp_info()
        if self.clnt.apiversion > 3.0:
            ...
            # self.cc_id = cc_id
            # self.set_ccid(cc_id)
            # self.cc_id = '3800b302-81ad-4815-a5f6-5f635b41979d'
            if methodToRun == 'change_control_create_for_tasks':
                ...
                print('Resp in if', chg_ctrl)
                return chg_ctrl
            else:
                print('ccid', self.cc_id)
                chg_ctrl_resp = methodToRun(self.get_ccid(), notes=cc_notes)
                print('REsp in else', chg_ctrl_resp)
    ...
    def test_api_change_control_create_for_tasks(self):
        ...
        chg_ctrl = self.change_control_opearions(
            'change_control_create_for_tasks')
        print('Result in calling ', chg_ctrl)
    ...
    def test_api_change_control_approve(self):
        ...
        # Approve the change control
        print('ccid in app', self.get_ccid())
        approve_note = "Approving CC via cvprac"
        approve_chg_ctrl = self.change_control_opearions(
    ...
    def test_api_change_control_start(self):
        ...
        start_note = "Start the CC via cvprac"
        # start_chg_ctrl = self.api.change_control_start(self.cc_id, notes=start_note)
        start_chg_ctrl = self.change_control_opearions(
    ...
    def test_api_change_control_stop(self):
        ...
        stop_note = "stop the CC via cvprac"
        # stop_chg_ctrl = self.api.change_control_stop(self.cc_id, notes=stop_note)
        stop_chg_ctrl = self.change_control_opearions(
